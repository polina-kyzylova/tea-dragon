class ItcSlider {
  ...
  #init() {
    this.#state.els = [];   // состояние элементов
    this.#state.translate = 0;   // текущее значение translate
    this.#state.activeItems = [];   // позиции активных элементов
    this.#state.isBalancing = false;   // состояние элементов
    const gap = parseFloat(getComputedStyle(this.#state.elItems).gap) || 0;   // получаем gap между слайдами
    this.#state.width = this.#state.elListItem[0].getBoundingClientRect().width + gap;   // ширина одного слайда
    const widthWrapper = this.#state.elWrapper.getBoundingClientRect().width;   // ширина #EL_WRAPPER
    this.#state.countActiveItems = Math.round(widthWrapper / this.#state.width);   // количество активных элементов
  ...
  #reset() {
    const transitionNoneClass = this.#state.prefix + this.constructor.#TRANSITION_NONE;
    const gap = parseFloat(getComputedStyle(this.#state.elItems).gap) || 0;   // получаем gap между слайдами
    const widthItem = this.#state.elListItem[0].getBoundingClientRect().width + gap;   // ширина одного слайда
